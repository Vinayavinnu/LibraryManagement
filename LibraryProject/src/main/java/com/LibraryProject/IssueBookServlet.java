...
public class IssueBookServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			  ...
			  List<BookEntity> booklist =bookDAO.listAvailableBooks();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) {
			...
			int bookId=Integer.parseInt(request.getParameter("category"));
			int userId=Integer.parseInt(request.getParameter("userId"));
			String description=request.getParameter("issueDescription");		
			
			BookDAO bookDAL = new BookDAO();
			int result = bookDAL.issueBook(bookId,userId,description);
			if(result==1)
			{
			...
			{
				request.setAttribute("Message", "User already have 3 books issued...");
			}
			else if(result==3)
			{
